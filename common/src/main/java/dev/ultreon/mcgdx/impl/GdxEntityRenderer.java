...
public class GdxEntityRenderer<T extends Entity> extends EntityRenderer<T> implements Gdx3DRenderSource<T> {
    ...
    private final ModelBatch batch = new ModelBatch(getShaderProvider(), new DefaultRenderableSorter() {

    });

    @NotNull
    protected DefaultShaderProvider getShaderProvider() {
        return new DefaultShaderProvider(getVertexShader(), getFragmentShader());
    }
    ...
    @Language("GLSL")
    protected @NotNull String getVertexShader() {
        return """
                #if defined(diffuseTextureFlag) || defined(specularTextureFlag) || defined(emissiveTextureFlag)
                #define textureFlag
                #endif
    ...
    public GdxEntityRenderer(EntityRendererProvider.Context context, Gdx3DRenderable renderable) {
        super(context);
